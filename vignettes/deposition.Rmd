...
- *DDEP_OXN_m2Grid* - deposition of oxidized nitrogen per m2 grid ($mgN/m^2$);
...
EMEP provides yearly, monthly, daily and hourly data. Reporting-cycle directories and file names change, so [get_atmo_dep](../reference/get_atmo_dep.html) accepts an explicit local or OPeNDAP NetCDF source for each year. `emep_2025_netcdf_source()` resolves the official 2025 Reporting dataset for 1990 through 2024. Obtain later reporting cycles from the [EMEP model-data page](https://www.emep.int/mscw/mscw_moddata.html). Depending on the selected time step, extraction can take some time. More information about atmospheric deposition in SWAT+ is presented in section 3.10 of the [SWAT+ modelling protocol](https://doi.org/10.5281/zenodo.7463395).
