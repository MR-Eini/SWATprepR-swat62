...
EMEP provides yearly, monthly, daily and hourly data. Reporting-cycle directories and file names change, so [get_atmo_dep](../reference/get_atmo_dep.html) requires an explicit local or OPeNDAP NetCDF source for each year. Obtain the current sources from the [EMEP model-data page](https://www.emep.int/mscw/mscw_moddata.html). Depending on the selected time step, extraction can take some time. More information about atmospheric deposition in SWAT+ is presented in section 3.10 of the [SWAT+ modelling protocol](https://doi.org/10.5281/zenodo.7463395).

```{r atmo_dep, eval=FALSE, message = FALSE, warning = FALSE}
library(SWATprepR)
library(tidyverse)
basin_path <- system.file("extdata", "GIS/basin.shp", package = "SWATprepR")
netcdf_source <- function(year, timestep) {
  file.path('path/to/emep', sprintf('EMEP_%s_%d.nc', timestep, year))
}
